...
const PropertyDetailPage = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${RESERVATIONS_URL}/property/${id}`)
      .then((res) => {
        console.log("sim ", res);
  ...
  const handleCancelProperty = async (id) => {
    ...
    const confirmCancel = window.confirm(
      "Tem certeza que deseja deletar esse imovel?"
    );

    if (confirmCancel) {
      await axios
        .delete(`${PROPERTIES_URL}/${id}`)
        .then((res) => {
  ...
  return (
    <div>
      ...
      {fromMyProperties === "true" ? <ReservaList reservas={reservas} /> : null}
    </div>
  );
};
